...
function conLista(stato, ...giocatori) {
  stato.desideri = giocatori.map((g) => g.id);
  return stato;
}

prova('senza lista i tetti restano quelli di prima', () => {
  const stato = statoNuovo();
  const g = DATI.giocatori.find((x) => x.r === 'A');
  assert.strictEqual(
    Conti.massimoPer(g, DATI, stato, PER_ID, STRATEGIE),
    Conti.massimoAdesso('A', DATI, stato, PER_ID));
});

prova('il costo atteso non sta mai sotto il prezzo del listone', () => {
  if (!STRATEGIE) return;
  const stato = statoNuovo();
  for (const g of DATI.giocatori.slice(0, 80)) {
    const costo = Conti.costoAtteso(g, STRATEGIE, stato.lega);
    assert.ok(costo >= Conti.prezzo(g, stato.lega),
      `${g.n}: costo atteso ${costo} sotto il prezzo`);
  }
});

prova('un desiderato può superare il tetto della casella media', () => {
  const stato = statoNuovo();
  const caro = DATI.giocatori.find((g) => g.r === 'A');
  const prima = Conti.massimoPer(caro, DATI, stato, PER_ID, STRATEGIE);
  conLista(stato, caro);
  const dopo = Conti.massimoPer(caro, DATI, stato, PER_ID, STRATEGIE);
  assert.ok(dopo > prima,
    `${caro.n}: in lista il tetto è ${dopo}, fuori era ${prima}`);
});

prova('mettere qualcuno in lista abbassa il tetto sugli altri del reparto', () => {
  const stato = statoNuovo();
  const attaccanti = DATI.giocatori.filter((g) => g.r === 'A');
  const voluto = attaccanti[0];
  const altro = attaccanti[5];
  const prima = Conti.massimoPer(altro, DATI, stato, PER_ID, STRATEGIE);
  conLista(stato, voluto);
  const dopo = Conti.massimoPer(altro, DATI, stato, PER_ID, STRATEGIE);
  assert.ok(dopo < prima,
    `${altro.n}: il tetto non è sceso (${prima} -> ${dopo})`);
});

prova('la lista non tocca i reparti che non la riguardano', () => {
  const stato = statoNuovo();
  const difensore = DATI.giocatori.find((g) => g.r === 'D');
  const prima = Conti.massimoPer(difensore, DATI, stato, PER_ID, STRATEGIE);
  conLista(stato, DATI.giocatori.find((g) => g.r === 'A'));
  const dopo = Conti.massimoPer(difensore, DATI, stato, PER_ID, STRATEGIE);
  assert.strictEqual(dopo, prima, 'un attaccante in lista ha mosso la difesa');
});

prova('chi è già stato preso non impegna più crediti', () => {
  const stato = statoNuovo();
  const voluto = DATI.giocatori.find((g) => g.r === 'A');
  const altro = DATI.giocatori.filter((g) => g.r === 'A')[5];
  conLista(stato, voluto);
  const conRiserva = Conti.riserva('A', DATI, stato, PER_ID, STRATEGIE);
  stato.venduti.push({ id: voluto.id, prezzo: 100 });
  const dopo = Conti.riserva('A', DATI, stato, PER_ID, STRATEGIE);
  assert.ok(conRiserva > 0 && dopo === 0,
    `riserva ${conRiserva} -> ${dopo} dopo che è stato venduto`);
  assert.ok(altro);
});

prova('un desiderato fermo non si riserva', () => {
  const stato = statoNuovo();
  const fermo = DATI.giocatori.find((g) => g.out && g.out.grave && g.r === 'C');
  conLista(stato, fermo);
  assert.strictEqual(Conti.riserva('C', DATI, stato, PER_ID, STRATEGIE), 0);
});

prova('nessun tetto supera mai i crediti che hai', () => {
  const stato = statoNuovo();
  const cari = DATI.giocatori.filter((g) => g.r === 'A').slice(0, 4);
  conLista(stato, ...cari);
  for (const g of DATI.giocatori.filter((x) => x.r === 'A').slice(0, 12)) {
    const tetto = Conti.massimoPer(g, DATI, stato, PER_ID, STRATEGIE);
    assert.ok(tetto <= Conti.residuo(stato),
      `${g.n}: tetto ${tetto} sopra il residuo ${Conti.residuo(stato)}`);
    assert.ok(tetto >= 0);
  }
});

prova('la sostenibilità dice quando la lista non ci sta', () => {
  const stato = statoNuovo();
  const modesta = Conti.sostenibilita(DATI, stato, PER_ID, STRATEGIE);
  assert.strictEqual(modesta.quanti, 0);
  assert.strictEqual(modesta.costo, 0);
  assert.strictEqual(modesta.sfora, false);

  // Dieci fra i piu' cari del listone non stanno in 500 crediti.
  conLista(stato, ...DATI.giocatori.slice(0, 10));
  const eccessiva = Conti.sostenibilita(DATI, stato, PER_ID, STRATEGIE);
  assert.strictEqual(eccessiva.quanti, 10);
  assert.ok(eccessiva.costo > eccessiva.residuo,
    `la lista costa ${eccessiva.costo} su ${eccessiva.residuo}`);
  assert.strictEqual(eccessiva.sfora, true);
});

prova('la sostenibilità segnala più desiderati che caselle', () => {
  const stato = statoNuovo();
  const portieri = DATI.giocatori.filter((g) => g.r === 'P').slice(0, 5);
  conLista(stato, ...portieri);
  const esito = Conti.sostenibilita(DATI, stato, PER_ID, STRATEGIE);
  assert.strictEqual(esito.troppi.P, 5 - DATI.slot.P);
});
