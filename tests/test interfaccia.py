...
class TestListaDesideri(unittest.TestCase):
    """La lista deve cambiare i consigli, non solo segnare dei nomi."""

    def test_il_pannello_esiste(self):
        for identificativo in ('lista-cerca', 'lista-desideri',
                               'lista-sostenibilita', 'lista-nota'):
            with self.subTest(id=identificativo):
                self.assertIn(identificativo, LETTORE.ids)

    def test_i_tetti_passano_dal_calcolo_sul_singolo(self):
        """Il tetto mostrato deve essere massimoPer, non quello del reparto."""
        self.assertNotIn('massimoAdesso(g.r)', CODICE,
                         'un tetto ignora ancora la lista dei desideri')
        self.assertIn('massimoPer(g)', CODICE)

    def test_la_lista_viene_salvata_e_ripristinata(self):
        self.assertIn('desideri', CODICE)
        # Deve finire sia nel salvataggio locale sia nella copia di backup.
        self.assertIn('desideri: stato.desideri', CODICE)
        self.assertIn('letto.desideri', CODICE)

    def test_annullare_ripristina_anche_la_lista(self):
        self.assertIn('passato.desideri', CODICE,
                      "l'annulla non rimette a posto la lista")
